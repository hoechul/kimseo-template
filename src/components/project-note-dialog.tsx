...
export interface ProjectNoteEditorValues {
  title: string;
  content: string;
  link_url: string;
}

interface ProjectNoteDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  note: ProjectNote | null;
  projectId: string;
  onSave: (values: ProjectNoteEditorValues) => Promise<boolean>;
}

const IMAGE_BUCKET = "project-note-images";
...
export function ProjectNoteDialog({
  ...
  const uploadImage = useCallback(
    ...
    [insertNodeAtCursor, projectId, supabase]
  );
      ...
      <DialogContent className="flex h-[95vh] max-h-[95vh] max-w-[95vw] flex-col overflow-hidden sm:max-w-[95vw]">
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col gap-4">
          <div className="space-y-2 shrink-0">
            <Label htmlFor="project-note-title">제목</Label>
            <Input
              id="project-note-title"
              value={title}
              onChange={(event) => setTitle(event.target.value)}
              placeholder="예: 고객 요청 정리"
              autoFocus
            />
          </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col space-y-2">
            <div className="flex shrink-0 items-center justify-between gap-2">
              <Label htmlFor="project-note-content">내용</Label>
          ...
          <div className="space-y-2 shrink-0">
            <Label htmlFor="project-note-link">관련 링크</Label>
            <Input
              id="project-note-link"
              value={linkUrl}
