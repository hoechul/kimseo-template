...
import { ExternalLink, FolderOpen, ImagePlus, Loader2, Pencil, Trash2, Users } from "lucide-react";
...
interface NoteEditorDialogProps {
  ...
  note: Note | null;
  onSave: (values: NoteEditorValues) => Promise<boolean>;
}
...
export function NoteEditorDialog({
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      const [{ data: projectsData }, { data: customersData }] = await Promise.all([
        supabase
          .from("projects")
  ...
  useEffect(() => {
    ...
    setCustomerId(note?.customer_id ?? null);
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (uploadError) {
          ...
          toast.error(`이미지 업로드에 실패했습니다. (${uploadError.message})`);
          return;
        }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const nextValues: NoteEditorValues = {
      title: title.trim(),
      content: sanitized,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[95vh] max-h-[95vh] max-w-[95vw] flex-col overflow-hidden sm:max-w-[95vw]">
        <DialogHeader className="shrink-0">
          ...
          <DialogDescription>
            아이디어나 메모를 작성하세요. 필요시 프로젝트나 고객에 연결할 수 있습니다.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col gap-4">
            ...
            >
              ...
              {isEditorEmpty ? (
                <p className="pointer-events-none absolute left-3 top-2 text-sm text-muted-foreground">
                  내용을 입력하세요. 이미지를 붙여넣거나 드래그&드롭으로 삽입할 수 있습니다.
                </p>
              ) : null}
              {isDraggingImage ? (
          ...
          <div className="grid grid-cols-1 gap-4 shrink-0 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setProjectId(v === "_none" ? null : v)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="연결할 프로젝트 선택" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="_none">연결 안함</SelectItem>
                  {projects.map((p) => (
                    <SelectItem key={p.id} value={p.id}>
                      [{p.project_number}] {mask("title", p.name)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setCustomerId(v === "_none" ? null : v)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="연결할 고객 선택" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="_none">연결 안함</SelectItem>
                  {customers.map((c) => (
                    <SelectItem key={c.id} value={c.id}>
                      {mask("customer_name", c.name)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <div className="space-y-2 shrink-0">
            <Label htmlFor="note-link">관련 링크</Label>
            <Input
              id="note-link"
              value={linkUrl}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90vh] max-w-2xl flex-col overflow-hidden">
        <DialogHeader className="shrink-0">
          <DialogTitle className="pr-8">{note.title ? mask("title", note.title) : "(제목 없음)"}</DialogTitle>
          <DialogDescription>
            {mask("name", note.author_name)} · {formatDate(note.created_at)}
            {note.updated_at !== note.created_at && ` (수정 ${formatDate(note.updated_at)})`}
          </DialogDescription>
        </DialogHeader>

        <div className="flex-1 overflow-y-auto space-y-4">
          {(note.projects || note.customers) && (
            <div className="flex flex-wrap gap-2">
              {note.projects && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-sm text-primary">
                  <FolderOpen className="h-3.5 w-3.5" />
                  [{note.projects.project_number}] {mask("title", note.projects.name)}
                </span>
              )}
              {note.customers && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-500/10 px-3 py-1 text-sm text-blue-600">
                  <Users className="h-3.5 w-3.5" />
                  {mask("customer_name", note.customers.name)}
                </span>
              )}
            </div>
          )}
          ...
          {note.content && (
            <div
              className="prose prose-sm dark:prose-invert max-w-none rounded-lg border border-border/50 bg-muted/30 p-4"
          ...
          {note.link_url && (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-4 py-3 text-sm text-primary hover:bg-muted/50"
            >
              <ExternalLink className="h-4 w-4" />
              <span className="truncate">{note.link_url}</span>
            </a>
          )}
        </div>

        <DialogFooter className="shrink-0 gap-2 sm:gap-0">
          <Button type="button" variant="destructive" onClick={onDelete} className="gap-1.5">
